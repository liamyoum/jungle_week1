...
if __name__ == '__main__':
    ...
    quotess=["빨리 가려면 혼자 가고, 멀리 가려면 함께 가라. — 아프리카 속담",
             "혼자서 할 수 있는 일은 작지만, 함께 할 수 있는 일은 위대하다. — 헬렌 켈러",
             "모이는 것은 시작이고, 함께 머무는 것은 진보이며, 같이 일하는 것은 성공이다. — 헨리 포드",
             "교향곡을 혼자서 휘파람으로 불 수는 없다. 그것을 연주하려면 오케스트라가 필요하다. — H.E. 루콕",
             "우리는 모두 한 날개로만 나는 천사들이다. 서로를 껴안아야만 날 수 있다. — 루치아노 데 크레센초",
             "우정이란 누군가에게 ‘뭐라고? 너도 그래? 나만 그런 줄 알았는데!’라고 말하는 순간 태어난다. — C.S. 루이스",
             "고난은 진정한 친구를 가려내는 시험대다. — 아리스토텔레스",
             "진정한 친구는 세상 모두가 나갈 때 우리 안으로 들어오는 사람이다. — 월터 윈첼"
             ,"누군가와 고통을 나누는 것은 그 고통을 반으로 줄이는 것이 아니라, 견딜 수 있는 힘을 두 배로 만드는 것이다. — 작자 미상 (유명 격언)",
             "우리는 서로의 용기가 되어야 한다. — 마야 안젤루",
             "천재성은 혼자서 빛날 수 있지만, 승리는 팀워크와 지성이 모여야 가능하다. — 마이클 조던",
             "재능은 게임에서 이기게 하지만, 팀워크와 이해력은 챔피언을 만든다. — 마이클 조던",
             "개미는 작지만 모이면 사자를 이긴다. — 에티오피아 속담",
             "스스로 빛을 내는 별보다, 서로를 비추는 별들이 더 밝은 법이다.",
             "당신이 다른 사람의 배를 강 건너로 저어다 주면, 당신도 어느덧 강 건너에 도착해 있을 것이다. — 인도 속담"]
