...
def start_time():
    ...
    return render_template('starttime.html',nowtime=nowtime)
...
def giime():
    ...
    target_user = db.user.find_one({'std_id': testid}, {'_id':0})
    endstudy = request.args.get('endstudy', True)
    ...
    return render_template('endtime.html', total_time=totaltimeret,this_Sestime=thisSestime,today_times=todaytimes)
...
def load_leaderboard():
    ...
    endleader = request.args.get('endleader', True)
    ...
    if len(leaderboard)>30 :
        return render_template('leaderboard.html',board=leaderboard[:30],myranking=myrank,myleaderboard=myleader)
    else :
        return render_template('leaderboard.html',board=leaderboard,myranking=myrank,myleaderboard=myleader)
...
def profileshow():
    ...
    if 'replys' in target_user:
        ...
        return render_template('profile.html',profile_inf=target_user,replys='')
    
    
    

    return render_template('profile.html',profile_inf=target_user,replystexts=replys)
