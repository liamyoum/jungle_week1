...
def generate_dummy_data():
    print("🗑️ 1단계: 기존 데이터를 완전히 삭제하여 충돌을 방지합니다...")
    
    # 기존 컬렉션 데이터 모두 삭제 (인덱스는 유지됨)
    db.user.delete_many({})
    ...
    print("✅ 기존 데이터 삭제 완료!\n")
    print("🚀 2단계: 실감 나는 사람 이름과 01~31 아이디로 더미 데이터 생성을 시작합니다...")
    ...
    # 2. 31명의 현실적인 이름 리스트 (아이디 01~31에 맞춤)
    korean_names = [
        "김민수", "이지은", "박지훈", "최수진", "정민호", 
        "강지영", "조현우", "윤서연", "장도윤", "임하은", 
        "한건우", "오지유", "서준서", "신지아", "권우진", 
    ...
    # "01"부터 "31"까지의 두 자리 숫자 형태의 ID 생성
    user_ids = [f"{i:02d}" for i in range(1, 32)]
    ...
    # 리스트를 순회하며 데이터 만들기
    for std_id, nickname in zip(user_ids, korean_names):
        ...
        user = {
            ...
            "nickname": nickname,
            "password": hashed_pw,
            "start_time": None, # 현재 측정 중이 아님
            "total_time": random_total_time,
            "todaytimes": [],   # 빈 배열로 초기화 (에러 방지 핵심)
            "friends": [],
            "ban_id": [],
    ...
    print(f"✅ {inserted_count}명의 새로운 유저 생성 완료 (아이디: 01~31, 비밀번호: 1234)")

    # 3. 랜덤하게 친구 추가 및 방명록(댓글) 작성
    print("🔄 3단계: 친구 관계 및 방명록 생성 중...")
    ...
    for std_id in user_ids:
        ...
        for _ in range(reply_count):
            writer = random.choice(user_ids) # 방명록 작성자 (아이디 형태: "01", "02" 등)
    ...
    print("🎉 완벽합니다! 기존 로그인 세션도 모두 초기화되었으니, 새로 로그인 후 타이머를 테스트해 보세요.")
